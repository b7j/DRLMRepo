...
def getSiteDetails(siteDetails,transectDetails,workbook):

    #pdb.set_trace()

    centrePointEasting = float(transectDetails.cell_value(6,1))
     
    centrePointNorthing = float(transectDetails.cell_value(6,4))
    
    UTMzone = transectDetails.cell_value(7,1)
    
    date = siteDetails.cell_value(6,1)
    ...
    observer = siteDetails.cell_value(3,1)
    
    recorder = siteDetails.cell_value(2,1)
    
    siteid = siteDetails.cell_value(5,1)
...
def mainRoutine():
    ...
    for i in filelist:
          ...
          siteDetailSheet = workbook.sheet_by_name(worksheets[1])

          transectDetailsSheet = workbook.sheet_by_name(worksheets[4])
          
          transectNSSheet = workbook.sheet_by_name(worksheets[5])

          transectSENWSheet = workbook.sheet_by_name(worksheets[6])

          transectNESWSheet = workbook.sheet_by_name(worksheets[7])

          #basalSheet = workbook.sheet_by_name(worksheets[6])

          #pdb.set_trace()

          latitude, longitude, date, observer, recorder, siteid = getSiteDetails(siteDetailSheet,transectDetailsSheet,workbook)
          ...
          northSouth = sendToOutput(latitude,longitude,date,siteid,observer,recorder,worksheets[5],glns,mlns,ulns)

          output.append(northSouth)

          southEast = sendToOutput(latitude,longitude,date,siteid,observer,recorder,worksheets[6],glse,mlse,ulse)
          
          output.append(southEast)

          northEast = sendToOutput(latitude,longitude,date,siteid,observer,recorder,worksheets[7],glne,mlne,ulne)
